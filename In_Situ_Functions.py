...
class InSitu:
    ...
    def Translate(_di, _dj, _W, _T, _M, _path):
        ...
        # This functions was built to fix an imaging error
...
class Lookup:
    ...
    """
    @staticmethod
    def Choose_Barcodes_V1(_df_reads, _df_lib, verbose=True):
        ...
        final_columns = ['sgRNA', 'cell', 'i_nuc', 'j_nuc']
        df_out = pd.DataFrame(data=np.empty([0, len(final_columns)]), columns=final_columns)

        for i, c in enumerate(cell_list):
            # print(i, '---------------------------------')
            df_cell = _df_reads[_df_reads['cell'] == c]
            # print(df_cell['barcode'])
            df_match = pd.DataFrame(data=np.empty([0, len(lib_column_names)]), columns=lib_column_names)
            for seq in df_cell['barcode']:

                check = _df_lib[seq == _df_lib['barcode']]

                if len(check) == 1:

                    df_match = pd.concat((df_match, check))
            ...
            if len(df_match) == 1:
                if verbose:
                    print(df_match)
                data_choice_final = np.array([[df_match['sgRNA'].iloc[0]], [c], [df_cell['i'].iloc[0]], [df_cell['j'].iloc[0]]], dtype=object)
            ...
            if len(df_match) > 1:
                if verbose:
                    print('Too many matches found, ambiguous')
        ...
        #    [[choice_final['sgRNA'].iloc[0]], [c], [df_cell['i'].iloc[0]], [df_cell['j'].iloc[0]],
        #     [int(choice_final['mismatch'].iloc[0])]], dtype=object)
        #df_choice_final = pd.DataFrame(data=np.transpose(data_choice_final), columns=final_columns)
        #df_out = pd.concat((df_out, df_choice_final), ignore_index=True)

    @staticmethod
    def Choose_Barcodes_V2(_df_reads, _df_lib, _nuclie, _cells, choose_barcode_method='popular', verbose=False):
        ...
        def Choose_Barcode_L3_mismatch(_choices_L3, _verbose=verbose):
            ...
            return _choice_Final, len(len(_choices_L4))
        ...
        for i, c in enumerate(cell_list):
            ...
            if not(isinstance(choice_final, type(None))):
                ...
                # data_choice_final = np.array([[choice_final['sgRNA'].iloc[0]], [c], [_df_series['I'].iloc[0]], [_df_series['J'].iloc[0]], [int(choice_final['mismatch'].iloc[0])], [choice_final['sgRNA_seq'].iloc[0]]], dtype=object)

                data_choice_final = np.array(
                    [[choice_final['sgRNA'].iloc[0]], [c], [_df_series['I'].iloc[0]], [_df_series['J'].iloc[0]], [len(df_cell)], [len(choices_L3)], [number_correct_reads],
    ...
    def Choose_Barcodes_No_Masks(_df_reads, _df_lib, choose_barcode_method='popular', verbose=False):
        ...
        #    txt_report = open(to_file_path, 'w+')
        ...
        report_columns = ['cell', 'comment_1', 'comment_2', 'comment_3', 'comment_4', 'assigned', 'raw reads',
                          'raw unique', 'processed unique']
        df_cell_report = pd.DataFrame(data=np.empty([0, len(report_columns)]), columns=report_columns)
        ...
        final_columns = ['sgRNA', 'cell', 'mismatch', 'barcode']
        df_out = pd.DataFrame(data=np.empty([0, len(final_columns)]), columns=final_columns)
        ...
        for i, c in enumerate(cell_list):
            ...
            if verbose:
                print(i, 'Cell:', c, '###############################################')
            # if to_file_path != None:
            #     txt_report.write(str(i) + ' Cell: ' + str(c) + '###############################################\n')
            ...
            if verbose:
                ...
                print('-----------------------------------------------')
            # if to_file_path != None:
            #    txt_report.write('Barcodes in cell:' + str(freq))
                ...
                if match_max < n_cyc - Mistake_lim:
                    ...
                    if len(choices_L2) == 0:
                        ...
                        if verbose:
                            print('Possible matches:')
                            for p in range(len(choices_L2)):
                                print('Barcode:', choices_L2['barcode'].iloc[p], 'Mismatch:',
                                      choices_L2['mismatch'].iloc[p], 'sgRNA:', choices_L2['sgRNA'].iloc[p])
            ...
            if verbose:
                ...
                print('All possible matches in cell:')
                for p in range(len(choices_L3)):
                    print('Barcode:', choices_L3['barcode'].iloc[p], 'Mismatch:', int(choices_L3['mismatch'].iloc[p]),
                          'sgRNA:', choices_L3['sgRNA'].iloc[p])
            ...
            if not (isinstance(choice_final, type(None))):
                ...
                if verbose:
                    print('-----------------------------------------------')
                    print('Choosen:', choice_final['barcode'].iloc[0], 'Mismatch:',
                          int(choice_final['mismatch'].iloc[0]), 'sgRNA:', choice_final['sgRNA'].iloc[0])
            ...
            report_data = np.array(
                [[c], [comment[0]], [comment[1]], [comment[2]], [comment[3]], [assigned], [len(df_cell)],
                 [len(freq.index)], [len(choices_L3['sgRNA'].value_counts().index)]], dtype=object)
            df_cell_report_add = pd.DataFrame(data=np.transpose(report_data), columns=report_columns)
            df_cell_report = pd.concat((df_cell_report, df_cell_report_add), ignore_index=True)
            if verbose:
                print('\n\n')
    ...
    @staticmethod
    def Choose_Barcode_Single_Cell(_well, _tile, _cell, _df_reads, _df_lib, verbose=False):

        pd.set_option('display.max_columns', None)

        # _df_reads = pd.read_csv(join(_path,'Reads/Reads_tile_' + str(_tile) + '_well_' + str(_well) + '.csv'))
        # _df_reads = pd.read_csv(_path)

        _df_cell = _df_reads[_df_reads['cell'] == _cell]

        if verbose:
            if len(_df_cell) == 0:
                print('Cannot find cell, cell has no reads')
            ...
            df_out, df_cell_report = Lookup.Choose_Barcodes_No_Masks(_df_cell, _df_lib, verbose=False)

            # _df_report = pd.read_csv(join(_path, 'Report_Genotype/Report_Genotype_tile_' + str(_tile) + '_well_' + str(_well) + '.csv'))

            # _df_report = _df_report[_df_report['cell'] == _cell]
    ...
    def Choose_Barcodes(_df_reads, _df_lib, _nuclie, _cells, N_lim=None, verbose=False):
        ...
        final_columns = ['sgRNA', 'cell', 'i_nuc', 'j_nuc', 'reads', 'matched', 'correct', 'mismatch_max',
                         'mismatch_min', 'barcode']
        df_out = pd.DataFrame(data=np.empty([0, len(final_columns)]), columns=final_columns)
        ...
        for i, c in enumerate(cell_list):
            ...
            for r, seq in zip(df_cell['read'], df_cell['barcode']):
                ...
                if len(choices_L2) == 0:
                    choices_L2 = pd.DataFrame(np.expand_dims(np.repeat(['No'], len(choices_L2.columns)), 0),
                                              columns=choices_L2.columns)
            ...
            if verbose:
                ...
                for i in range(len(choices_L3)):
                    try:
                        short_list[i] = choices_L3['sgRNA'].iloc[i].split('_')[2] + '_' + \
                                        choices_L3['sgRNA'].iloc[i].split('_')[4]
                    except:
