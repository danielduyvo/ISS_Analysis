...
def Crop_Cell_Window(_I, _J, _cell_mask):
    ...
    # _I: i coordinate position of cell
    # _J: j coordinate position of cell
    # _cell_mask: cellular mask of tile containing cell
    # return: i min/max j min/max of box for cropping cell
...
def Expand_Mask(_crop, deg=5):
    # Roll out cell mask, useful for cropping cell exhibiting cellular membrane phenotype
    # deg: size of structural elements, controls the extent of the mask expansion
...
def Erode_Cell(_crop, deg=3):
    # Roll back cell mask, useful for cropping cell exhibiting cellular membrane phenotype
    # deg: size of structural elements, controls the extent of the mask erosion
...
def Get_Last_2D(data):
    ...
    # extract last two dimensions of numpy array
...
def Select_Album_Cells(_df_geno_sgRNA, _n_cell_album, method='random'):
    ...
    # Function that helps selecting cells from an album
    # _df_geno_sgRNA: dataframe of genotyped cells
    # _n_cell_album: number of cells in album
    # method: random, selects cells randomly. brightest, selects brightest cells
    # return: reduced dataframe contain cells for album only
...
def Blue_Black_CMAP():
    ...
    # colormap that uses shading from back to blue
...
def Normalize_Enhance_Constrast(_img):
    ...
    # normalize of flattened image, and clean of zero value pixels
...
def Make_Cell_Image_From_CSV(I_, J_, _img, _mask, nuc_mask=np.array([None]), padding=0, erode=None, crop_background=True):
    ...
    # Function that creates a cell crop from phenotyped cells dataframe
    # I_: i coordinate of cell mask
    # J_: j coordinate of cell mask
    # _img: high mag image containing cell
    # _mask: cellular segmented mask containing cell
    # nuc_mask: nuclear segmented mask containing cell
    # padding: number of pixel layer of value zero to add around box of cell
    # erode: roll in cellular mask
    # crop_background: Use cellular mask to eliminate neighboring cells
    # return single image, with masks
...
def Outline(_I, _J, _mask, padding=0):
    ...
    # Calculate x and y arrays of cell outline
    # I_: i coordinate of cell mask
    # J_: j coordinate of cell mask
    # _mask: cellular mask of tile containing cell
    # padding: number of pixel layer of value zero to add around box of cell
...
def Outline_From_Mask(_single_cell):
    ...
    # Calculate x and y arrays of cell outline from cellular mask
    # _single_cell: single cell mask
...
def Find_Guides_With_Localizations(_df_geno, _df_local, _loc_name):
    ...
    # sgRNA names with a certain localization pattern
    # _df_geno: genotyped cells dataframe
    # _df_local: dataframe with localization patterns
    # _loc_name: localization pattern of interest
    # return: list of sgRNAs names
...
def Save_Cell_Image(_img_cell_final, _cell_name, _save_path):
    ...
    # Save single images without white boundaries
    # _img_cell_final: numpy array or cell image
    # _cell_name: Name of cell picture save file
    # _save_path: path to save directory
